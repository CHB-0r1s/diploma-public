"""Resumable mean assistant-token entropy scoring for SFT data selection."""
...
def select_highest_entropy(scores: np.ndarray, k: int) -> np.ndarray:
    """Return dataset-sorted indices of the top-k finite entropy scores."""
    values = np.asarray(scores)
    valid_indices = np.flatnonzero(np.isfinite(values))
    if len(valid_indices) < k:
        raise ValueError(f"Only {len(valid_indices)} valid entropy scores remain for k={k}")
    ranked = np.argsort(-values[valid_indices], kind="stable")[:k]
    return np.sort(valid_indices[ranked])
...
class EntropySelector:
    ...
    def __post_init__(self) -> None:
        if self.tokenizer.padding_side != "right":
            raise ValueError("Entropy scoring requires right-padding")
    ...
    def score(self, conversations: Sequence[Conversation]) -> np.ndarray:
        ...
        progress = tqdm(total=len(pending), desc=f"entropy (batch={self.batch_size})")
        since_save = 0
        for start in range(0, len(pending), self.batch_size):
            indices = pending[start : start + self.batch_size]
            batch = [conversations[index] for index in indices]
            try:
                batch_scores = self._entropy_batch(batch)
            except Exception as exc:
                print(
                    f"\n[!] Entropy batch idx={indices[0]}..{indices[-1]}: "
        ...
        print(
            f"Entropy scoring complete. NaN {missing}/{len(scores)} "
            f"({100 * missing / max(len(scores), 1):.2f}%)"
        )
        return scores

    @torch.no_grad()
    def _entropy_batch(self, conversations: Sequence[Conversation]) -> list[float]:
        texts = [
        ...
        if not hasattr(logits, "shape"):
            raise RuntimeError("Entropy scorer did not return tensor logits")
        ...
        for batch_index in range(len(conversations)):
            ...
            for chunk_start in range(0, len(logit_positions), self.token_chunk_size):
                positions = logit_positions[
                    chunk_start : chunk_start + self.token_chunk_size
                ]
                token_logits = logits[batch_index, positions].float()
                log_normalizer = torch.logsumexp(token_logits, dim=-1)
                expected_logit = (
                    torch.softmax(token_logits, dim=-1) * token_logits
                ).sum(dim=-1)
                entropy_sum += (log_normalizer - expected_logit).sum()
            results.append(float((entropy_sum / len(logit_positions)).item()))
        return results

    def select(self, scores: np.ndarray, k: int) -> np.ndarray:
        return select_highest_entropy(scores, k)

    def _cache_path(self) -> Optional[str]:
        return os.path.join(self.cache_dir, "scores_entropy.npy") if self.cache_dir else None
    ...
    def _load_partial(self, size: int) -> np.ndarray:
        ...
        if path and os.path.exists(path):
            ...
            if len(scores) != size:
                raise RuntimeError(f"Entropy cache size {len(scores)} does not match {size}")
            complete = int(np.isfinite(scores).sum())
            print(f"Found {complete}/{size} cached entropy scores in {path}")
